...
// HID keycodes that we care about for this game
...
#define FIELD_X ((CANVAS_W/2)-(BLOCKS_W/2)*BLOCK_SIZE)
#define FIELD_W (BLOCKS_W*BLOCK_SIZE)
#define FIELD_Y ((CANVAS_H/2)-(BLOCKS_H/2)*BLOCK_SIZE)
#define FIELD_H (BLOCKS_H*BLOCK_SIZE)

// saves color of block, if any, at field[x][y]
static uint16_t field[BLOCKS_W][BLOCKS_H];

// where to draw stuff
#define KEYS_X (CANVAS_W/8)
#define KEYS_Y (CANVAS_H/5)
#define NEXT_X ((3*CANVAS_W/4)+BLOCK_SIZE)
#define NEXT_Y (CANVAS_H/8)
#define LEVEL_X ((3*CANVAS_W/4)+BLOCK_SIZE)
#define LEVEL_Y (2*CANVAS_H/4)
#define SCORE_X ((3*CANVAS_W/4)+BLOCK_SIZE)
#define SCORE_Y (3*CANVAS_H/4)
...
static shape shapes[] = {
    {15, // 0: white bar
        {0b0100010001000100,  //   0
         0b0000111100000000,  //  90
         0b0100010001000100,  // 180
         0b0000111100000000}},// 270
    {13, // 1: magenta tee
        {0b0100111000000000,  //   0
         0b0100110001000000,  //  90
         0b0000111001000000,  // 180
         0b0100011001000000}},// 270
    {12, // 2: blue backward-L
        {0b0100010011000000,  //   0
         0b0000111000100000,  //  90
         0b0110010001000000,  // 180
         0b1000111000000000}},// 270
    {14, // 3: cyan forward-L
        {0b0100010001100000,  //   0
         0b0010111000000000,  //  90
         0b1100010001000000,  // 180
         0b0000111010000000}},// 270
    {11, // 4: yellow cube
        {0b1100110000000000,  //   0
         0b1100110000000000,  //  90
         0b1100110000000000,  // 180
         0b1100110000000000}},// 270
    {9, // 5: red forward-Z
        {0b0100110010000000,  //   0
         0b1100011000000000,  //  90
         0b0100110010000000,  // 180
         0b1100011000000000}},// 270
    {10, // 6: green backward-Z
        {0b1000110001000000,  //   0
         0b0000011011000000,  //  90
         0b1000110001000000,  // 180
...
{
    uint8_t i, j;
    // erase whatever is on display
    for (i = 0; i < BLOCKS_W; i++) {
        for (j = 0;j < BLOCKS_H; j++) {
            field[i][j] = 0;
        }
    }
    fill_rect(0, FIELD_X, FIELD_Y, FIELD_W, FIELD_H);
...
static void update_level()
{
    ...
    fill_rect(0 /*black*/, LEVEL_X+5*BLOCK_SIZE, LEVEL_Y, 4*6, 8);
...
static void update_score()
{
    ...
    fill_rect(0 /*black*/, SCORE_X+5*BLOCK_SIZE, SCORE_Y, 4*6, 8);
...
static void update_paused()
{
    if (paused) {
        ...
        fill_rect(0 /*black*/, 0, canvas_height()-8, 17*6, 8);
    }
}
...
static void draw_background()
{
    ...
    set_text_multiplier(1);
    set_text_color(white());
    set_cursor(NEXT_X+BLOCK_SIZE/2, NEXT_Y-BLOCK_SIZE/2);
    draw_string("NEXT:");
...
void restart_game()
{
    ...
    current_x = FIELD_X + FIELD_W/2  - (current_shape==0?2:3)*BLOCK_SIZE;
    current_y = FIELD_Y + (current_shape==0?1:0)*BLOCK_SIZE;
    current_level = 1;
    current_score = 0;
    timer_threshold = 60;
...
void main()
{
    ...
    init_bitmap_graphics(0xFF00, 0x0000, 0, CANVAS_H==180?2:1, CANVAS_W, CANVAS_H, 4);
    ...
    while (1) {
        ...
        if (!paused && timer > timer_threshold) {
            ...
            // drop the crrent_shape if possible
            if (validate_move(current_rotation, current_x, current_y+BLOCK_SIZE)) {
                erase_shape(current_shape, current_rotation, current_x, current_y);
                current_y += BLOCK_SIZE;
                draw_shape(current_shape, current_rotation, current_x, current_y);
            } else { // can't drop the current_shape any more
                process_drop();
            }
        }
        ...
        if (!(keystates[0] & 1)) {
            if (!handled_key) { // handle only once per single keypress
                ...
                if (!paused && key(KEY_RIGHT)) { // try to move shape right
                    if (validate_move(current_rotation, current_x+BLOCK_SIZE, current_y)) {
                        erase_shape(current_shape, current_rotation, current_x, current_y);
                    ...
                    uint8_t new_rotation = (current_rotation+1)%4;
                    if (validate_move(new_rotation, current_x, current_y)) {
                        erase_shape(current_shape, current_rotation, current_x, current_y);
                        current_rotation = new_rotation;
                        draw_shape(current_shape, current_rotation, current_x, current_y);
                    }
                } else if (!paused && key(KEY_DOWN)) { // drop the shape as far as possible
                    while(validate_move(current_rotation, current_x, current_y+BLOCK_SIZE)) {
                        erase_shape(current_shape, current_rotation, current_x, current_y);
                        current_y += BLOCK_SIZE;
                        draw_shape(current_shape, current_rotation, current_x, current_y);
                    }
